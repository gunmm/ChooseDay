...
#define WHUCalendarView_TopView_Height 44.0f
...
@interface WHUCalendarView()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
{
    ...
    ZXYGtasksViewController *_gtasks;
    
    CalaViewController *_Vc;
}
...
//    /Users/mac/Desktop/GitHub/ChooseDay/ChooseDay/Assets.xcassets/Picture/开关箭头类/fun_ic_add_grey.imageset/fun_ic_add_grey@3x.png
    
    UIButton* preBtn=[self makeButtonWithText:@"添加"];
//    [preBtn setBackgroundImage:[[UIImage imageNamed:@"fun_ic_add_grey"] rt_tintedImageWithColor:[UIColor whiteColor] level:1] forState:UIControlStateNormal];
    ...
    _mLeftBtn=preBtn;
    preBtn.tag=1000;
    [preBtn addTarget:self action:@selector(addGtasks) forControlEvents:UIControlEventTouchUpInside];
    UIButton* nextBtn=[self makeButtonWithText:@"今天"];
    _mRightBtn=nextBtn;
    nextBtn.tag=2000;
    ...
    curDateLbl.textColor=[UIColor whiteColor];
...
- (void)addGtasks{
     ...
     _gtasks=[[ZXYGtasksViewController alloc]init];
    
    _gtasks.dataStr=_dataStrr;
    
    
    _Vc= (CalaViewController *)[self returnVC];
    
    [self addc];
    
    [_Vc.navigationController pushViewController:_gtasks animated:YES];
...
- (void)addc{
    ...
    if(self.selectedDate){
        ...
        NSString* string = [dateFormatter stringFromDate:self.selectedDate];
        
        _gtasks.dataStr=string;
        ...
        //yyyy-MM-dd HH-mm -ss  zz代表的时区
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString* string = [dateFormatter stringFromDate:nowDate];
...
-(void)monthSelectAction:(UIButton*)sender{
    if(sender.tag==1000){
        [self loadCalendarDataForNow:WHUCalendarViewPreMonth];
    }
    else{
        [self loadCalendarDataForNow:WHUCalendarViewNextMonth];
    }
}
...
-(void)loadCalendarDataForNow:(WHUCalendarViewMonthOption )option{
    WHUCalendarView_WeakSelf weakself=self;
    if(option==WHUCalendarViewPreMonth){
        [_calcal preMonthCalendar:_curMonthLbl.text complete:^(NSDictionary* dic){
            WHUCalendarView_StrongSelf self=weakself;
            ...
            [self reloadCollectionView];
               ...
               [_calcal nowMonthCalendar:_curMonthLbl.text complete:^(NSDictionary *dic) {
                   WHUCalendarView_StrongSelf self=weakself;
                   self->_dataDic=dic;
                   CATransition* tran=[CATransition animation];
                   self->_curMonthLbl.text=dic[@"monthStr"];
